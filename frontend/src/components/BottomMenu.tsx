...
export const BottomMenu = ({chosen}: Props) => {
    ...
    return <div className="fixed flex bottom-8 w-full justify-center">
        <div
            ...
            <img className={`${chosen === 'HOME' ? 'invert' : ''}`} src="/home.svg"
                 onClick={() => dispatch(updateView('HOME_PAGE'))}/>
            <img className={`${chosen === 'CHATS' ? 'invert' : ''}`} src="/chats.svg"
                 onClick={() => dispatch(updateView('CHATS'))}/>
            <img className={`${chosen === 'PROFILE' ? 'invert' : ''}`} src="/profile.svg"
                 onClick={() => dispatch(updateView('PROFILE'))}/>
        </div>
    </div>
}
